...
def calculate_session_duration_stats():
    ...
    for session in sessions:
        ...
        if duration <= 10:
            duration_brackets["0-10秒"] += 1
        elif duration <= 30:
            duration_brackets["10-30秒"] += 1
        elif duration <= 60:
            duration_brackets["30秒-1分钟"] += 1
        elif duration <= 180:
            duration_brackets["1-3分钟"] += 1
        elif duration <= 600:
            duration_brackets["3-10分钟"] += 1
        else:
            duration_brackets["10-30分钟"] += 1
...
def process_sessions(input_file, output_file1, output_file2):
    global sessions
    # 加载会话数据
    try:
